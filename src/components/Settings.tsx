...
import { Trash2, Plus, Users, CreditCard, Landmark, Tag as TagIcon, X, Edit2, Save, Archive, ArchiveRestore, Percent, Layout, CheckSquare, Square, BarChart3, Loader2, Briefcase, TrendingUp } from 'lucide-react';
import { generateUUID } from '../utils';

interface SettingsProps {
  accounts: Account[]; setAccounts: (item: any) => void;
  ...
  enabledModules: { cards: boolean; shared: boolean; budget: boolean; business: boolean; investments: boolean; };
  setEnabledModules: React.Dispatch<React.SetStateAction<{ cards: boolean; shared: boolean; budget: boolean; business: boolean; investments: boolean; }>>;
...
export const Settings: React.FC<SettingsProps> = ({ 
  ...
  const toggleModule = (module: 'cards' | 'shared' | 'budget' | 'business' | 'investments') => {
    ...
    <div className="space-y-10 pb-20 animate-fadeIn max-w-5xl mx-auto">
      <section className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* NOVOS MÓDULOS */}
            <div className={`p-4 rounded-xl border cursor-pointer flex items-center gap-3 ${enabledModules.business ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800' : 'bg-slate-50 border-slate-200 opacity-60'}`} onClick={() => toggleModule('business')}><div className={`p-2 rounded-full ${enabledModules.business ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-500'}`}><Briefcase size={20}/></div><div><div className="font-bold dark:text-white">Gestão Empresarial</div><div className="text-xs text-slate-500">{enabledModules.business ? 'Ativo' : 'Inativo'}</div></div>{enabledModules.business && <CheckSquare className="ml-auto text-indigo-600" size={20}/>}</div>
            <div className={`p-4 rounded-xl border cursor-pointer flex items-center gap-3 ${enabledModules.investments ? 'bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800' : 'bg-slate-50 border-slate-200 opacity-60'}`} onClick={() => toggleModule('investments')}><div className={`p-2 rounded-full ${enabledModules.investments ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-500'}`}><TrendingUp size={20}/></div><div><div className="font-bold dark:text-white">Investimentos</div><div className="text-xs text-slate-500">{enabledModules.investments ? 'Ativo' : 'Inativo'}</div></div>{enabledModules.investments && <CheckSquare className="ml-auto text-amber-500" size={20}/>}</div>
