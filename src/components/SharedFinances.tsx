...
interface SharedFinancesProps {
  ...
  onBatchSave?: (transactions: Transaction[]) => void; // Nova prop
}
...
export const SharedFinances: React.FC<SharedFinancesProps> = ({
  ...
  const [columnWidths, setColumnWidths] = useState<{[key: string]: number}>({
    select: 48, date: 120, description: 300, value: 130, category: 150, responsible: 120, mode: 120, actions: 120
  });
    ...
    <div className="flex flex-col h-full overflow-hidden animate-fadeIn">
        ...
        <div className="flex-1 overflow-x-auto overflow-y-auto bg-white dark:bg-slate-800 rounded-xl shadow border dark:border-slate-700 relative">
            <table className="min-w-full text-sm text-left table-fixed">
                <thead className="text-xs text-slate-500 bg-slate-50 dark:bg-slate-700 uppercase sticky top-0 z-10 shadow-sm">
                    <tr>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {sortedBills.map(bill => {
                        ...
                        return (
                            <tr key={bill.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700 ${selectedIds.has(bill.id) ? 'bg-indigo-50' : ''}`}>
                                ...
                                <td className="p-3 font-medium dark:text-white truncate" title={bill.description}>{bill.description}</td>
