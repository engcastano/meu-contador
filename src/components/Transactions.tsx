...
const Transactions: React.FC<TransactionsProps> = ({ 
  ...
  const [columnWidths, setColumnWidths] = useState<{[key: string]: number}>({
    select: 48, date: 120, description: 300, value: 130, type: 150, account: 150, status: 120, actions: 120
  });
  ...
  // Categoria Inteligente (Combobox)
  ...
  // --- CÁLCULO DE SALDO DO CARD (RESULTADO DO PERÍODO) ---
  const getAccountBalance = (accName: string) => {
      // Usa displayedTransactions (já filtradas por mês/ano) para mostrar apenas o fluxo do período
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden">
         ...
         <div className="hidden md:block h-full overflow-auto">
             <table className="min-w-full text-sm text-left table-fixed">
                 <thead className="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-700 dark:text-slate-400 sticky top-0 z-10">
                     <tr>
                 ...
                 <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                     {sortedTransactions.map(t => {
                         ...
                         return (
                         <tr key={t.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 ${selectedIds.has(t.id) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                             ...
                             <td className="px-4 py-3 font-medium dark:text-white truncate" title={t.description}>{t.description}</td>
