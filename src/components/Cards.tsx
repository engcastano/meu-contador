...
interface CardsProps {
  ...
  onBatchSave?: (newTxs: CardTransaction[]) => void;
}
...
export const Cards: React.FC<CardsProps> = ({ 
  ...
  const [columnWidths, setColumnWidths] = useState<{[key: string]: number}>({
    select: 48, datePurchase: 120, value: 130, type: 150, dateInvoice: 120, mode: 120, status: 100
  });
  
  const currentConfig = cardConfigs.find(c => c.id === selectedCardId);

  const minTableWidth = useMemo(() => {
      let total = Object.entries(columnWidths).reduce((acc, [key, w]) => {
          if (key === 'mode' && !currentConfig?.isShared) return acc;
          return acc + w;
      }, 0);
      return total + 300 + 100; // +300px Descrição +100px Ações
  }, [columnWidths, currentConfig]);
        ...
        <table className="min-w-full text-sm text-left table-fixed" style={{ minWidth: minTableWidth }}>
            ...
            <tbody>
                {sortedTransactions.map(t => { 
                    ...
                    return ( <tr key={t.id} className={`group hover:bg-slate-50 dark:hover:bg-slate-700/50 ${selectedIds.has(t.id) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-slate-500 truncate">{parseDate(t.datePurchase)}</td>
                        ...
                        <td className={`px-4 py-3 font-bold truncate ${valueColor}`}>{parseCurrency(t.value)}</td>
                        <td className="px-4 py-3 truncate"><span className="px-2 py-1 rounded text-xs text-white shadow-sm truncate inline-block max-w-full" style={{ backgroundColor: tagColor }}>{t.type}</span></td>
                        <td className="px-4 py-3 text-xs text-slate-500 truncate">{formatMonthYear(t.dateInvoice)}</td>
                        {currentConfig?.isShared && <td className="px-4 py-3 text-xs truncate">{sharingModes.find(m => m.id === t.sharingModeId)?.name || '-'}</td>}
                        <td className="px-4 py-3 text-center truncate"><span className={`px-2 py-1 rounded text-xs ${getInvoiceStatus(t) === 'Aberta' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>{getInvoiceStatus(t)}</span></td>
