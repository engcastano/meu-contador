...
import { SharedFinances } from './SharedFinances';
import { BudgetFlow } from './BudgetFlow';
import { Investments } from './Investments';

import { Moon, LayoutDashboard, List, CreditCard, Settings as SettingsIcon, Users, LogOut, Calendar, CheckCircle, AlertTriangle, BarChart3, Filter, Tag as TagIcon, Loader2, CloudOff, NotebookPen, X, TrendingUp, ArrowLeft } from 'lucide-react';
import { subscribeToCollection, saveData, deleteData, saveBatchData, getCollectionName, saveUserPreferences, getUserPreferences } from '../api';
...
export const PersonalApp: React.FC<PersonalAppProps> = ({ user, onBackToSelection }) => {
  ...
  // MODO PESSOAL: Removemos 'business' daqui. 'investments' mantido como opcional.
  const [enabledModules, setEnabledModules] = useState({ cards: false, shared: false, budget: true, investments: false, business: false });
  ...
  useEffect(() => {
      if (user.preferences && typeof user.preferences === 'object') {
          // Garante que carregamos as preferencias salvas, se existirem
  ...
  useEffect(() => {
    ...
    const unsubs = [
      subscribeToCollection(
        user.id, 
        getCollectionName('Transactions'), 
        (data) => setTransactions(data),
        where('dateExpected', '>=', startYear),
        where('dateExpected', '<=', endYear)
      ),
      subscribeToCollection(
        user.id, 
        getCollectionName('CardTransactions'), 
        (data) => setCardTransactions(data),
        where('dateInvoice', '>=', `${filters.year}-01-01`),
        where('dateInvoice', '<=', `${filters.year}-12-31`)
      ),
      subscribeToCollection(
        user.id, 
        getCollectionName('BudgetTargets'), 
        (data) => setBudgetTargets(data),
    ...
    if (enabledModules.investments) {
        unsubs.push(subscribeToCollection(user.id, getCollectionName('Investments'), setInvestments));
    }

    setTimeout(() => setIsLoadingData(false), 600);

    return () => unsubs.forEach(unsub => unsub());
  }, [user, filters.year, enabledModules]); 
  ...
  const tabs = useMemo(() => {
      ...
      // Investments mantido se ativado, Business removido
      if(enabledModules.investments) t.push({ id: 'investments', label: 'Invest', icon: TrendingUp });
  ...
  const handleSaveStructuralData = async (collectionKey: string, item: any) => {
    if (!user) return;
    await saveData(user.id, getCollectionName(collectionKey), { ...item, userId: user.id });
  ...
  const handleDeleteTransaction = (id: string) => requestConfirm('Excluir Transação', 'Tem certeza?', async () => {
    const currentUid = auth.currentUser?.uid;
    if(!currentUid) return;
    await deleteData(currentUid, getCollectionName('Transactions'), id);
    showToast('Excluído.');
  });
  ...
  const handleDeleteTransactionsById = (ids: string[]) => requestConfirm('Excluir', `Excluir ${ids.length} itens?`, async () => {
    const currentUid = auth.currentUser?.uid;
    if(!currentUid) return;
    const promises = ids.map(id => deleteData(currentUid, getCollectionName('Transactions'), id));
  ...
  const handleDeleteCardTransactionsById = (ids: string[]) => requestConfirm('Excluir', `Excluir ${ids.length} compras?`, async () => {
    const currentUid = auth.currentUser?.uid;
    if(!currentUid) return;
    const promises = ids.map(id => deleteData(currentUid, getCollectionName('CardTransactions'), id));
    await Promise.all(promises);
    showToast('Compras excluídas.');
  });
  ...
  const handleDeleteInvestment = (id: string) => requestConfirm('Excluir', 'Apagar investimento?', async () => {
    await deleteData(user!.id, getCollectionName('Investments'), id);
  });
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(t => {
       ...
       if (filters.status === 'realized' && !t.isRealized) return false;
       if (filters.status === 'predicted' && t.isRealized) return false;

       return true; 
    });
  }, [transactions, filters]);

  if (isLoadingData) return <div className="h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 flex-col gap-4"><Loader2 className="animate-spin text-blue-600 w-10 h-10"/><span className="text-slate-500">Sincronizando dados Pessoais...</span></div>;
    ...
    <div className="flex flex-col min-h-screen lg:h-screen lg:overflow-hidden bg-slate-50 dark:bg-slate-900 relative">
      ...
      {confirmModal.isOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
              <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-2xl max-w-sm w-full border border-slate-200 dark:border-slate-700 transform transition-all scale-100">
                  <h3 className="text-lg font-bold dark:text-white mb-2">{confirmModal.title}</h3>
                  <p className="text-slate-600 dark:text-slate-300 mb-6">{confirmModal.message}</p>
                  <div className="flex justify-end gap-3">
                      <button onClick={closeConfirm} className="px-4 py-2 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 font-medium transition-colors">Cancelar</button>
                      <button onClick={() => { confirmModal.onConfirm(); closeConfirm(); }} className="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 font-bold shadow-lg transition-transform active:scale-95">Confirmar</button>
                  </div>
              </div>
          </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        <aside className="hidden lg:flex w-64 flex-shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex-col p-4 overflow-y-auto gap-6">
             <div className="flex items-center gap-3 px-2 mb-2"><div className="bg-blue-600 p-2 rounded-lg"><NotebookPen className="text-white w-5 h-5" /></div><h1 className="text-lg font-bold leading-tight dark:text-white">Pessoal</h1></div>
             ...
             <div className="mt-auto space-y-2">
                <button onClick={onBackToSelection} className="w-full flex items-center gap-2 px-3 py-2 text-slate-600 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-800 rounded-lg text-sm transition-colors"><ArrowLeft size={16}/> Trocar Modo</button>
                <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg text-sm transition-colors"><LogOut size={16}/> Sair</button>
             </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-900 relative lg:overflow-hidden pb-20 lg:pb-0">
             ...
             <div className="flex-1 lg:overflow-y-auto p-4 lg:p-6">
                ...
                {activeTab === 'investments' && enabledModules.investments && (
                    <Investments 
                      investments={investments} 
                      onSave={(inv) => handleSaveStructuralData('Investments', inv)}
                ...
                {activeTab === 'settings' && <Settings 
                    accounts={accounts} 
                    setAccounts={(item) => handleSaveStructuralData('Accounts', item)} 
                    onHardDeleteAccount={hardDeleteAccount} 
                    tags={tags} 
                    setTags={(item) => handleSaveStructuralData('Tags', item)} 
                    onDeleteTag={handleDeleteTag} 
                    cardConfigs={cardConfigs} 
                    setCardConfigs={(item) => handleSaveStructuralData('CardConfigs', item)} 
                    onHardDeleteCard={hardDeleteCard} 
                    sharedAccounts={sharedAccounts} 
                    setSharedAccounts={(item) => handleSaveStructuralData('SharedAccounts', item)} 
                    hardDeleteSharedAccount={hardDeleteShared} 
                    sharingModes={sharingModes} 
                    setSharingModes={(item) => handleSaveStructuralData('SharingModes', item)} 
